...
export class ModulomenuComponent implements OnInit {
  ...
  constructor(private registropublicacionesService: RegistroPublicacionService, private obtenerpublicacionService: ObtenerPublicacionService) { 
  ...
  ngOnInit() {
    ...
    /*for ( const i in this.publicaciones) {
        $('.apartado').append('<div class="objeto3"><h6>{{this.publicaciones[i].plataforma}} </h6></div>');
     }
     */
